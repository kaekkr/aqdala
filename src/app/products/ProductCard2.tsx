...
const ProductCard = ({ name, description, img }: IProductCard2) => {
	return <div className='space-y-2 md:space-y-3 flex flex-col items-center text-center'>
		...
		<div className='lg:p-6 p-2 lg:space-y-6 space-y-2'>
			...
			<p className='md:hidden p3 max-w-[18rem]' style={{ marginTop: 0 }}>{name}</p>
			<p className='md:hidden p3 text-neutral-dark max-w-[9rem] md:max-w-[19rem]'>{description}</p>
			<p className='hidden lg:block p2-m text-neutral-dark max-w-[9rem] md:max-w-[19rem]'>{description}</p>
			<p className='hidden md:block lg:hidden p2-s text-neutral-dark max-w-[9rem] md:max-w-[19rem]'>{description}</p>
		</div>
	</div>
}
